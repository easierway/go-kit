...
def default_balance_factor(fm="consul/factor_map.json", consul="localhost:8500"):
    ...
    status, output = subprocess.getstatusoutput("curl -s http://100.100.100.200/latest/meta-data/instance/instance-type")
    machine = "unknown"
    if status == 0:
...
def default_zone():
    status, output = subprocess.getstatusoutput("curl -s http://100.100.100.200/latest/meta-data/zone-id")
    zone = "unknown"
    if status == 0:
        zone = output.split()[0]
    return zone
...
def default_instanceID():
    status, output = subprocess.getstatusoutput("curl -s http://100.100.100.200/latest/meta-data/instance-id")
    instanceID = "unknown"
    if status == 0:
...
def default_publicIP():
    status, output = subprocess.getstatusoutput("curl -s http://100.100.100.200/latest/meta-data/eipv4")
    publicIP = "unknown"
    if status == 0:
...
""",
    ...
    parser.add_argument("-I", "--instanceID", type=str, help="instanceID")
    parser.add_argument("-v", "--publicIP", type=str, help="publicIP")
